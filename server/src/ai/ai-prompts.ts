...
const extractProtectedTokens = (
	...
	const tokens = raw.flatMap(
		value => value.match(/[A-Za-z][A-Za-z0-9-]*/g) ?? []
	)
...
export const buildImproveDescriptionPrompt = (
	...
	return [
		...
		'10) Стиль: нейтральный, фактический, деловой, без воды.',
		'11) Длина message: 450-900 символов.',
		'12) Минимум 5 полноценных предложений.',
		...
		'14) В первом предложении используй шаблон: "Продается <title>".',
		'15) Нельзя менять тип объекта из title/input (например, таунхаус -> квартира).',
		'16) В абзаце 3 перечисли минимум 2 неуказанных параметра, которые важно уточнить перед покупкой.',
		'17) Если входных фактов мало, пиши коротко и прямо, не заполняй текст оценочными выводами.',
		'18) Пиши только в логике продажи: это объявление продавца, а не запрос покупателя.',
		'19) Прямо запрещены формулировки спроса: "куплю", "ищу", "нужен", "приобрету", "рассмотрю покупку".',
		'20) Даже если во входном тексте встречается формулировка спроса, преобразуй ее в корректную форму продажи без потери фактов.',
		'21) Прямо запрещены рекламные и оценочные штампы: "уникальная возможность", "отличная возможность", "идеально подходит", "выгодное расположение", "современные стандарты", "уютная".',
		'22) Нельзя приписывать объекту назначение, преимущества, состояние, инфраструктуру и локационные плюсы, если этого нет в input/allowed_facts.',
		'23) Перед отправкой проверь себя: если нарушено хоть одно правило, перепиши ответ заново.',
...
export const buildSuggestPricePrompt = (
	...
	return [
		...
		'   "Рекомендованная цена: <число> ₽. Диапазон: <число>-<число> ₽. Уверенность: <низкая|средняя|высокая>. Обоснование: <ровно 3 предложения>."',
		'5) Структура 3 предложений в обосновании обязательна:',
		'   1-е: связь с входной ценой; 2-е: что видно по данным поиска; 3-е: ограничение/риск + финальный вывод.',
		...
		'9) Если status=not_available или priceSignalsCount=0, Уверенность строго "низкая" без исключений.',
		'10) Если рыночных сигналов мало или они противоречивы, объясни это явно.',
		'11) Если входной price есть, используй его как якорь и отклоняйся только при явной причине.',
		'12) Не добавляй технические факты об объекте, которых нет во входе.',
		'13) Не интерпретируй цену как "оптовая", "розничная", "премиальная" и т.п., если это не указано во входе.',
		`14) Разрешенные числа из входа: ${allowedNumbers.join(', ') || 'нет'}. Новые числа допустимы только для ценовой рекомендации и диапазона.`,
		'15) Не используй мусорные формулировки и канцелярит.',
		'16) Прямо запрещены формулировки: "предложений ограничено", "количество ограничено", "есть несколько предложений".',
		'17) Никаких английских предложений и транслита.',
		'18) Нельзя упоминать официальных дилеров, автосалоны и розничные каналы, если этого нет прямо во входе и market_research.',
		'19) Не используй язык покупателя и спроса ("куплю", "ищу", "нужен", "приобрету").',
		'20) Не используй буквальную формулировку "По market_research". Пиши естественно: "по данным поиска", "по доступным рыночным данным".',
		'21) Если status=not_available, в обосновании прямо укажи, что оценка предварительная и может измениться после проверки рынка.',
		'22) Длина message: 320-650 символов.',
		'23) Перед отправкой проверь себя: если нарушено хоть одно правило, перепиши ответ заново.',
		...serializeInput(input),
		...serializeAllowedFacts(input),
